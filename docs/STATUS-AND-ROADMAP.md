...
## 1. Where vfkb sits (the one-paragraph north-star)

vfkb is two things at once:
...
> One line (FEATURES §1): *vfkb turns one-shot agents into a team with a memory.*
...
comes from context; context is what vfkb supplies. vfkb is the front-half lever.
...
- Repo `vilosource/vfkb` `main` @ `0e75823`; package `0.1.0`; `tsc` clean; **95/95 tests**.
...
- **Decisions ADR-0001–0023** ratified; **RFC-001–006** drafted (001/002/004/005/006 accepted;
  003 proposed + evidence-gated). vfkb now **dogfoods its own brain** (ADR-0019).
...
| Project context doc | **SHIPPED 2026-06-28** (Track-4b D-ii / ADR-0025 ← RFC-007): the assembled per-project **context document** — authored spine (`<brain>/context.md`) + derived Constitution/Map/decisions — read on demand via the `kb_context` MCP tool + CLI `vfkb context` (FEATURES §3.7 / D-O8). `kb-context-first-read` pi/claude 3/3 |
| Self-hosted design-brain | vfkb dogfoods its own per-project tier — committed `.vfkb/` (ADR-0019): ADR/RFC link-index + native gotchas/patterns |
...
- The **devops-kb live dogfood** (vfkb as a real DevOps agent's operating memory)
...
- **Self-hosted design-brain (ADR-0019, RFC-004):** vfkb commits its own `.vfkb/` and
...
- **Dogfood check-6 flake hardened:** deterministic in-image `tools/list` preflight (6a) +
  bounded LLM retry (6b) — proven green on host and in the `vfkb-dogfood` image.
...
- ✅ **`tool-gating`** — closed without a paid rerun: it's deterministically unit-tested
  ...
  at HEAD).
- ✅ **Corpus self-pollution** — fixed (`31f4266`): Tier-B capture now skips vfkb's own
  `kb_*`/`mcp__vfkb__*` tools; regression-tested.
- ✅ **"BM25-lite" comments** — corrected to "stemmed term-overlap (no IDF/length-norm)".
- ✅ **Self-hosted design-brain** (ADR-0019) — vfkb commits its own `.vfkb/` and dogfoods
...
- ⬜ **De-dupe seeded entries** — remaining, but it's *spike seed data* (the devops-kb
  `migrate-seed`), not vfkb core; defer or handle in the spike, not here.
...
across `vfkb-DESIGN` (D1–D7), `project-onboarding-schema-DESIGN` (D-O1–O8, the #1↔#2
contract), `IngestEngine/*` (the pipeline), and ADRs 0001–0016.

H1's *real*, much smaller residue:
- **Supersede the stale brainstorming doc** (`vfsf-ingest-and-vfkb-DESIGN.md`) — point
...
- **Genuinely-open design residue** (small): the kb-**write** methodology specifics
  (how architect/executor/judge *capture* into vfkb — the execution-side `architect.md`/
  `executor.md`/`judge.md` already exist in vafi but don't write knowledge yet).

### H2 — Ingest integration *(OUT OF SCOPE for current vfkb development — 2026-06-06)*
> **Scope note (2026-06-06):** integration into **vafi / vtaskforge is out of scope for
> vfkb development at this time.** vfkb work stays inside the `vfkb` repo. The plan
> below is kept as the *designed-and-ready* target for when fleet integration is picked
> up; only the **vfkb-side** residue (already done — see H2a step 1) belonged here.
> The active vfkb frontier is **H0** + **H4** (both in-repo). Embedding `VFKB_ROLE`
> attribution (the one vfkb-side prerequisite) shipped (`860cab8`).

vfkb is "the substrate the ingest agents stand on." The design is ready; the work is
...
- **H2a — Wire vfkb into the live fleet *(smallest, highest-leverage; do first)*.**
  The attachment points are known and narrow: add `vfkb` alongside `vtf`/`cxdb` in
  `vafi/config/bridge-roles.yaml` (`mcp_tools`), thread `VF_VFKB_MCP_URL`/token in
...
- **H2b — Onboarding + ingest engine *(larger; designed-not-built)*.** Build project
  ...
  where the *missing front half* gets built; it spans vtaskforge + vafi, not just vfkb.
...
The vfkb-served global instance (REST + MCP + web UI), project→global gated
...
```
   IN SCOPE (vfkb repo)            │   OUT OF SCOPE for now (vafi / vtaskforge)
...
H2's one vfkb-side prerequisite (`VFKB_ROLE` attribution) already shipped (`860cab8`).
So the active vfkb work is **H4, sequenced in
...
doc's* forks, closed since by `vfkb-DESIGN` D1–D7 and `project-onboarding-schema-DESIGN`:

| Was-listed-as-open | Actually |
|---|---|
| Global-tier transport (git vs API) | **Resolved** — DESIGN D2b: per-project git-local; global = git SoR + vfkb-served REST/MCP/web UI (deferred to H3) |
...
| Python/dir-name | **Settled** — TypeScript (ADR-0002/0003); `.vfkb/` (D2c) |
...
|---|---|---|---|
| vfkb engine (per-project tier) | `vfkb-DESIGN` D1–D7 + ADRs 0001–0016 | locked | ✅ v1 built |
...
> **`vfsf-ingest-and-vfkb-DESIGN.md` is superseded** by the rows above; it remains as
...
`vfsf-ingest-and-vfkb-DESIGN.md` (superseded brainstorm), the verified vafi/vtaskforge
