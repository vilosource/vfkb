# H2a — Wire vfkb into the live agent fleet (implementation plan)

> **Status:** PLAN — PARKED (2026-06-06). The **vafi-side steps (§2, §4, §5) are OUT OF
> SCOPE for current vfkb development**; `vilosource/vafi` is not being touched at this
> time. Kept as the designed-and-ready target for when fleet integration is in scope.
> **Done:** the one vfkb-side prerequisite — `VFKB_ROLE` attribution (§3.1) — shipped
> in `860cab8`. Grounded against the verified vafi file shapes (read 2026-06-06) and the
> locked vfkb DESIGN (D2b/D2c/D2d/D7c).

## 0. Goal & scope

**Goal:** a fleet agent reads + writes its project's vfkb brain via MCP, gets
...
seeds the brain. H2a uses a tiny pre-seeded `.vfkb` to prove the *path*, not the content.
...
**vfkb is different:** per DESIGN **D2b/D2c/D2d**, the per-project brain is
**git-repo-local** (`<main-repo>/.vfkb`), read **locally**. So in v1 vfkb attaches as a
**stdio** MCP server — `node /opt/vfkb/dist/mcp-server.js` with `VFKB_DIR=$WORKDIR/.vfkb`
— exactly the devops-kb spike's `mcp-config.json`. The HTTP-served vfkb is the **global
tier**, deferred to H3.
...
`/opt/vfkb`, mirroring the devops-kb spike (no registry dependency):
```dockerfile
...
minimal skeleton (`vfkb init`) so the server has a valid brain — full seeding is H2b.

**2.3 Claude face** (`entrypoint.sh` `~/.claude.json` patch): add a **stdio** entry beside
the http `vtf`/`cxdb`:
```python
cfg.setdefault('mcpServers', {})['vfkb'] = {
    'type': 'stdio', 'command': 'node',
    'args': ['/opt/vfkb/dist/mcp-server.js'],
    'env': {'VFKB_DIR': workdir + '/.vfkb',
            'VFKB_PROJECT': project_slug,
            'VFKB_ROLE': agent_role},
}
```
Plus a vfkb **settings.json** (SessionStart inject + PostToolUse capture, the devops-kb
template) passed via `claude --settings` so injection/capture fire.

**2.4 Pi face** (`pi_config.py`): add a stdio `vfkb` server to the `mcpServers` dict.
...
**2.5 Role attribution** (both faces + `pi_session.py:build_pi_env`): pass `VFKB_ROLE`
...
**2.6 Role config** (`config/bridge-roles.yaml`): add `vfkb` to `mcp_tools` for
`architect` (and the executor/judge agent configs).

## 3. Changes — `vilosource/vfkb` (small, do first)
...
- **3.1** Default `author.role` from **`process.env.VFKB_ROLE`** in the MCP `kb_add`/
  ...
  test: `VFKB_ROLE=architect` → a write lands `author.role=architect` without the model
  passing it.
...
Seed a tiny `.vfkb` in a throwaway repo, launch the path (architect pod first, or the
headless harness), and **assert external effects**:
1. session-start injects a `<vfkb-context>` block;
2. `kb_search` returns the seeded entry (relevance-primary, post-ADR-0016);
3. a `kb_add` write lands **attributed to the pod's role** (set via `VFKB_ROLE`, *not*
   model-reported);
...
1. **vfkb 3.1/3.2** (role-from-env) — unit-testable in vfkb alone.
...
- **Greenfield empty brain** — needs a clean `vfkb init`; the real seed is H2b/onboarding.
