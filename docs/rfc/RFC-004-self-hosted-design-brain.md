# RFC-004: vtfkb self-hosts its own design-brain (commit `.vtfkb/`; ADRs link-not-copy)
...
- **Refines (on acceptance):** applies [D2c](../DESIGN.md) (brain single-homed at
  `<main-repo>/.vtfkb`) and [D1 constraint 4](../DESIGN.md) (docs integrate via the
  `link` primitive, not by copying content) **to vtfkb's own repo**. Dogfoods
...
vtfkb's stated identity is "the shared memory the other three products' agents stand
on" (STATUS-AND-ROADMAP §1) and "the substrate the ingest agents stand on." Yet **vtfkb
...
single-homed and git-committed at `<main-repo>/.vtfkb`. The vtfkb repo has **no committed
`.vtfkb/`** — only `.vtfkb-spike/`, which `.gitignore` discards as a throwaway. So:

- An agent (human or LLM) working **on** vtfkb cannot query vtfkb-the-substrate for
  vtfkb's own decisions, gotchas, or patterns. It must read `docs/` by hand.
- vtfkb's hard-won **native operating knowledge** — the relevance-floor bug
  ...
  commit messages. It has **no agent-consumable home**, which is precisely the gap vtfkb
  exists to close for *other* projects.
- vtfkb therefore fails to dogfood its own D2c / per-project tier — the same dogfooding
...
*"`docs/` integrate via the `link` primitive (extended to repo-relative paths). vtfkb owns
the links/index, **not the file content**."* The D1 ownership table is explicit: vtfkb holds
...
vtfkb adopts its **own per-project tier**: a committed `.vtfkb/` brain in the vtfkb repo,
populated and maintained so the substrate serves vtfkb's own knowledge.

1. **Commit `.vtfkb/entries.jsonl` (+ the project context doc); keep the index out.**
...
3. **The brain's `decision`/`fact`/`gotcha`/`pattern` entries carry only vtfkb's
   *native* knowledge** — the operating lessons that have no markdown home today
   ...
   This is the knowledge an agent working on vtfkb actually needs and currently cannot get
   from the substrate.

4. **Seed from the existing engine path, not by hand.** The brain is populated via the
   engine's own write API / the `migrate-seed` mechanism (`VTFKB_SEED_AREAS`,
   commit `1143fb8`) so the act of self-hosting exercises vtfkb's own seed + write +
...
5. **No secrets, low-trust** ([D1 constraint 2](../DESIGN.md)). The committed brain holds
   ...
   it. vtfkb's design corpus has no secrets, so this is satisfied by construction.
...
6. **Proposed, not speculative — but cheap and directly actionable.** Per
   ...
   **accept and build** (commit the brain + seed the native entries + the ADR/RFC link
   index), then let the substrate accrue vtfkb's knowledge from here forward.

## Consequences

- **+** vtfkb finally **stands on itself** — agents working on vtfkb query the substrate
...
- **+** Self-hosting exercises seed + write + lint + rebuild on a real, growing brain —
  continuous dogfood beyond the spike harness; regressions surface against vtfkb's own data.
...
- **Neutral:** the global tier (D2a/H3), promotion (D2f), and area model (D2e) are
  untouched — this is purely the per-project tier applied to one more project (vtfkb itself).
...
- **Status quo — no committed brain; agents read `docs/` directly.** Rejected — vtfkb keeps
...
- **Keep it in `~/.vtfkb` (host home), uncommitted.** Rejected — that is the spike default;
  it is not shared, not reviewable, not version-controlled, and not what D2c specifies
  (`<main-repo>/.vtfkb`). It cannot serve a *team* (or the next session) memory.

## Related

[D1](../DESIGN.md) (scope: vtfkb owns links/index to docs, not content — constraint 4;
git-committed/no-secrets — constraint 2), [D2b/D2c/D2g](../DESIGN.md) (per-project tier is
git-repo-local, single-homed at `<main-repo>/.vtfkb`, the v1 scope),
...
+ `VTFKB_SEED_AREAS` (commit `1143fb8`), `.gitignore` (currently discards `.vtfkb-spike/`).
