...
- **Supersedes:** the prior position (vtfkb-DESIGN D6a, original) that vtfkb would
  be written in **Python**. That position is preserved below under *Alternatives*
  as the superseded reasoning.

## Context

vtfkb needs three faces over one engine: an **MCP server** (the cross-harness
...
Implement vtfkb in **TypeScript.** One TS engine (storage/index/git/scoring) with
...
- **Neutral:** vtfkb does not match the Django/Python orchestration stack — but it
...
- **Python (the superseded prior decision).** Original rationale: (1) "the
  ...
  weak because vtfkb is a separate repo that need not match the orchestration
