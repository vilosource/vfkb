...
assumed, and does not bind.** The bridge needs zero porting, and has been **observed working in
the dockerized `pi -e` L4 arm** — stated that narrowly on purpose, because under ADR-0051
clause 1 `-e <path>` is exactly the load path this ADR calls a trap, so "it works in pi" is the
one phrasing this document must not use loosely. What is proven is the bridge's *capability*
under that harness; its behaviour once installed as a package is what milestone 1 must show.
Concretely: 106 self-contained lines, loaded like any pi extension, calling one `ExtensionAPI` method
...
reinvented. And the pi arm **sidesteps the credential blocker RFC-036 documents**: the Claude
scenarios read `~/.claude/.credentials.json` and **throw** without a `claudeAiOauth` block
(`scenarios/l4-purpose.mjs:84-86`, and identically in the plugin's `hooks-smoke.mjs:74-80` and
`install-path.mjs:110-115`), with no API-key code path anywhere — which is why those records
can only be produced by hand on the operator's machine. The pi arm instead runs on a plain
API key, `DEEPSEEK_TOKEN`, **passed to the container by the harness at run time**
(`scenarios/l4-purpose.mjs:278`) and consumed via the baked provider config, whose `apiKey`
field names the env var rather than holding a literal (`scenarios/docker/pi.Dockerfile:27-30`).
...
> its prize. The pi install-path L4 must assert the credential is present before it runs.

**Bad.** A third repo to keep in step, with the vendoring drift ADR-0045 already lives with.
pi moves fast — 0.61.0 shipped 2026-03-20 and 0.73.1 on 2026-05-07: twelve minor bumps in about
seven weeks (pi's `CHANGELOG.md:3,940`) — so a package pinned against a stub-typed API
re-incurs the mykb L7 failure on every pi release. That is why `pi.Dockerfile`'s pin and the
