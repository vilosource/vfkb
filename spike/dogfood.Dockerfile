# vfkb container dogfood — a clean-room Claude Code session in the deployment-shaped
...
# 2. vfkb substrate: built artifact + the single runtime dep (@modelcontextprotocol/sdk,
...
# 3. Clean-room config (vfkb-only; ZERO mykb).
...
#    silent-write-failure on bind mounts. The repo (/work/vfkb) and brain (/brain)
#    mounts are owned by the host uid that --user matches.
RUN mkdir -p /work/.claude /brain && chmod -R 0777 /work /brain

ENV HOME=/work VFKB_DIR=/brain VFKB_PROJECT=vfkb-dogfood
WORKDIR /work/vfkb
