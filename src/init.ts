...
function hookCommand(project: string, sub: string): string {
  return `VFKB_DIR=.vfkb VFKB_PROJECT=${project} node ${BOOTSTRAP_REL} cli hook ${sub}`;
}
...
// engine via $VFKB_HOME at runtime; degrades GRACEFULLY (clear, actionable
// message; never breaks the session) when VFKB_HOME is unset or the bundles are
// missing. DO NOT hand-edit — regenerated by \`vfkb init\`.
...
const home = process.env.VFKB_HOME;
const engine = mode === 'mcp' ? 'vfkb-mcp.mjs' : 'vfkb.mjs';
const enginePath = home ? join(home, engine) : '';

const FIX =
  'vfkb is INACTIVE: VFKB_HOME is not set (or its bundles are missing). ' +
  'Fix: build the bundles in the vfkb repo (\\\`npm run build:bundles\\\`) and ' +
  '\\\`export VFKB_HOME=/path/to/vfkb/dist/bundles\\\` (see docs/CONSUMER-ONBOARDING.md). ' +
...
(\`dist/bundles\`), e.g. \`export VFKB_HOME=/path/to/vfkb/dist/bundles\`. If it is unset, a session-start
banner will tell you; run \`node .vfkb/bin/bootstrap.mjs cli doctor\` (or \`vfkb doctor\`) to check.
`;
}
